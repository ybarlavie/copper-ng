...
const _sendQRCode = (email, secret) => {
    return new Promise((resolve, reject) => {
        ...
        QRCode.toDataURL(qrCodeUrl, function (err, url) {
            ...
            jwtUrl += `<br /><p>או, לחלופין, ליצור חשבון בשם <span style="font-size:24px; color: green;">Copper-NG(${email})</span> ולהזין את הקוד הבא בשדה המפתח שלך:<br /><br />הקוד: <span style="font-family: 'Courier New', monospace; font-size: 24px; color: red;">${secret}</span>`;
